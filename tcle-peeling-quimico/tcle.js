...
const CONFIG = Object.freeze({
  API_URL: 'https://rjxtxoqprnumouqakxbc.supabase.co/functions/v1/tcle-peeling-quimico-submit',
  PUBLISHABLE_KEY: 'sb_publishable_Ip14p4tUfYFjwUYaWinMlw_Gf9v0YwT',
  TYPE: 'tcle_peeling_quimico',
  TERM_VERSION: '2026-08-18-v1',
  TERM_SHA256: '3f04db1af1f0e984385862abdd0b1b93ae34ebfd49f09587f541c317022b530b',
  RELEASED: true,
...
function validateForm() {
  ...
  if (!CONFIG.RELEASED) return { ok: false, message: 'Este documento não está disponível para envio.' };
...
function collectData(signedAt) {
  const modalities = ['Peeling químico superficial não fenólico'];
...
function showSuccess(response, data) {
  ...
  state.pdfFilename = String(response.pdf_nome || '').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 100) ||
    'Pre-Avaliacao-Peeling-Quimico-Superficial-' + safeFilename(data.identificacao.nome) + '-' + response.codigo.slice(0, 8) + '.pdf';
  ...
  const whatsappText = 'Olá, Ana! Concluí a pré-avaliação do TCLE de peeling químico superficial pelo site. Nome: ' + data.identificacao.nome + '. Código: ' + response.codigo.slice(0, 16) + '.';
...
form.addEventListener('submit', async (event) => {
  event.preventDefault();
  if (!CONFIG.RELEASED) {
    showError('Este documento não está disponível para envio.');
    return;
  }
...
if (!CONFIG.RELEASED) {
  ...
  document.getElementById('form-status').textContent = 'Este documento não está disponível para envio.';
}
