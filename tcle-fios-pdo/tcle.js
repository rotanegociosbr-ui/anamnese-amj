...
const CONFIG = Object.freeze({
  API_URL: 'https://rjxtxoqprnumouqakxbc.supabase.co/functions/v1/tcle-fios-pdo-submit',
  PUBLISHABLE_KEY: 'sb_publishable_Ip14p4tUfYFjwUYaWinMlw_Gf9v0YwT',
  TYPE: 'tcle_fios_pdo',
  TERM_VERSION: '2026-08-18-v1',
  TERM_SHA256: '6a31a13133e29132763e2ded44c6d1a424d64154d0cb883b02628d897924bd24',
  RELEASED: true,
...
function renderTerm(text) {
  ...
  target.innerHTML = blocks.map((block, index) => {
    ...
    if (index < 2) return '<p class="term-title">' + esc(block) + '</p>';
...
function validateForm() {
  ...
  if (!CONFIG.RELEASED) return { ok: false, message: 'Este documento está temporariamente indisponível para envio.' };
...
function collectData(signedAt) {
  const modalities = ['Fios absorvíveis de PDO'];
...
function showSuccess(response, data) {
  ...
  state.pdfFilename = String(response.pdf_nome || '').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 100) ||
    'Pre-Avaliacao-Fios-PDO-' + safeFilename(data.identificacao.nome) + '-' + response.codigo.slice(0, 8) + '.pdf';
  ...
  const whatsappText = 'Olá, Ana! Concluí a pré-avaliação do TCLE de fios de PDO pelo site. Nome: ' + data.identificacao.nome + '. Código: ' + response.codigo.slice(0, 16) + '.';
...
form.addEventListener('submit', async (event) => {
  event.preventDefault();
  if (!CONFIG.RELEASED) {
    showError('Este documento está temporariamente indisponível para envio.');
    return;
  }
...
if (!CONFIG.RELEASED) {
  ...
  document.getElementById('form-status').textContent = 'Este documento está temporariamente indisponível para envio.';
}
