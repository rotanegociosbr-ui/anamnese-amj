...
function setupInputs() {
  ...
  form.querySelectorAll('input[name="modalidades"]').forEach((input) => {
    input.addEventListener('change', () => {
      if (!form.querySelector('input[name="modalidades"]:checked')) return;
      document.getElementById('erro-modalidades').textContent = '';
...
function validateForm() {
  ...
  const modalities = [...form.querySelectorAll('input[name="modalidades"]:checked')];
  if (!modalities.length) {
    document.getElementById('erro-modalidades').textContent = 'Escolha ao menos uma modalidade de interesse.';
    return { ok: false, message: 'Informe ao menos uma modalidade de interesse.', element: form.querySelector('input[name="modalidades"]') };
  }
  document.getElementById('erro-modalidades').textContent = '';
...
function collectData(signedAt) {
  const modalities = [...form.querySelectorAll('input[name="modalidades"]:checked')].map((input) => input.value);
...
function showSuccess(response, data) {
  ...
  state.pdfFilename = String(response.pdf_nome || '').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 100) ||
    'Pre-Avaliacao-Intradermoterapia-' + safeFilename(data.identificacao.nome) + '-' + response.codigo.slice(0, 8) + '.pdf';
  ...
  const whatsappText = 'Olá, Ana! Concluí a pré-avaliação do TCLE de intradermoterapia estética pelo site. Nome: ' + data.identificacao.nome + '. Código: ' + response.codigo.slice(0, 16) + '.';
