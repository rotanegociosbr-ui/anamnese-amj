...
const TYPE = "tcle_fios_pdo";
const TERM_VERSION = "2026-08-18-v1";
const TERM_SHA256 =
  "6a31a13133e29132763e2ded44c6d1a424d64154d0cb883b02628d897924bd24";
const RELEASED = true;
...
const LOGO_URL =
  "https://anamariajacob.com.br/assets/identidade-visual-transparente-v1.png";
const LOGO_SHA256 =
  "6dd5a16fa082b9875882ad795f975eff3a1423fc4198937a1fc4f955584d33ff";
...
const HEALTH_QUESTIONS = Object.freeze([
  "Você está gestante, pode estar gestante, está tentando engravidar ou amamentando",
  "Você está com febre, gripe, infecção ou outra doença aguda",
  "Há ferida, inflamação, herpes, acne ativa ou doença de pele na região pretendida",
  "Você tem sangramento fácil, coagulopatia ou histórico de hematomas importantes",
  "Você usa anticoagulante, antiagregante, aspirina, anti-inflamatório ou suplemento que possa alterar a coagulação",
  "Você tem alergia ou reação anterior a fios de sutura ou PDO, anestésico, antisséptico, adesivo, látex ou metal",
  "Você tem doença autoimune, imunidade comprometida ou usa imunossupressor ou corticoide continuamente",
  "Você tem diabetes, hipertensão, doença cardíaca ou outra condição crônica",
  "Você está em tratamento de câncer, quimioterapia ou radioterapia",
  "Você tem tendência a queloide, cicatriz elevada ou dificuldade de cicatrização",
  "Você já colocou fios, preenchimentos ou implantes, ou realizou cirurgia ou procedimento na área",
  "Você já teve infecção, granuloma, extrusão, assimetria, alteração nervosa ou outra complicação após procedimento estético",
  "Há infecção ou tratamento odontológico recente ou programado quando a região pretendida é facial",
]);
...
function matchesExistingOwner(
  previous: JsonRecord,
  payload: JsonRecord,
): boolean {
  return normalizeName(text(previous.nome, 120)) ===
      normalizeName(text(payload.nome, 120)) &&
...
function safeData(
  ...
  if (
    !identification || !procedure || !image || !declarations ||
  ...
  if (
    !isAdult(birth) || !emergency || local.length < 2 || innerEmail !== email
  ) return null;
  if (
  ...
  if (
    emergencyName.length < 2 || emergencyRelation.length < 2 ||
    !/^\d{10,11}$/.test(emergencyPhone)
  ) return null;

  if (
    !Array.isArray(procedure.modalidades) || procedure.modalidades.length < 1 ||
    procedure.modalidades.length > 3
  ) return null;
  const modalities = procedure.modalidades.map((item) => text(item, 50));
  if (
    !modalities.every((item) => ALLOWED_MODALITIES.has(item)) ||
  ...
  if (!photo || !disclosure || !beforeAfter || !firstName || !testimonial) {
    return null;
  }
  if (disclosure === "nao") {
    if (
      beforeAfter !== "nao" || firstName !== "nao" || testimonial !== "nao" ||
      display !== "nao_aplicavel"
    ) return null;
...
async function officialLogo(): Promise<Uint8Array> {
  if (!officialLogoPromise) {
    officialLogoPromise = (async () => {
      ...
      if (!response.ok) {
        throw new Error("official_logo_unavailable_" + response.status);
      }
      const bytes = new Uint8Array(await response.arrayBuffer());
      if (
        bytes.byteLength > 500_000 || !isPng(bytes) ||
        await sha256(bytes) !== LOGO_SHA256
      ) {
...
async function buildCanonicalPdf(
  ...
  pdf.setTitle(
    "TCLE - Fios Absorvíveis de PDO - " + data.identificacao.nome,
  );
  pdf.setSubject(
    "Manifestação eletrônica do paciente - aguardando revisão profissional",
  );
  ...
  const pdfText = (value: unknown): string =>
    Array.from(String(value == null ? "" : value))
      .map((character) =>
  ...
  const wrap = (
    ...
    const output: string[] = [];
    for (
      const rawParagraph of pdfText(value).replace(/\r\n?/g, "\n").split("\n")
    ) {
      ...
      for (const word of words) {
        ...
        for (const character of Array.from(word)) {
          ...
          if (
            fragment && font.widthOfTextAtSize(candidateFragment, size) > width
          ) {
  ...
  textBlock("FIOS ABSORVÍVEIS DE POLIDIOXANONA (PDO)", {
    font: bold,
    size: 11,
    lineHeight: 15,
    after: 4,
  ...
  textBlock(
    "CÓPIA DO ENVIO DO PACIENTE — AGUARDANDO REVISÃO E ASSINATURA PROFISSIONAL",
    {
      font: bold,
      size: 8.2,
      lineHeight: 11,
  ...
  textBlock("MANIFESTAÇÃO INICIAL — NÃO AUTORIZA IMPLANTAÇÃO", {
    font: bold,
    size: 9.4,
    lineHeight: 13,
    after: 12,
  ...
  const termBlocks = termText.split(/\n{2,}/).map((item) => item.trim()).filter(
    Boolean,
  );
  for (let index = 0; index < termBlocks.length; index++) {
  ...
  textBlock(
    "Documento gerado no servidor e vinculado ao termo " + TERM_VERSION +
      " (SHA-256 " + TERM_SHA256 + "). Recebido em " +
      formatServerDate(receivedAt) + ".",
  ...
  textBlock(
    "Data de nascimento: " + data.identificacao.nascimento + "   ·   CPF: " +
      data.identificacao.cpf,
  );
  textBlock(
    "Telefone: " + data.identificacao.telefone +
      (data.identificacao.email
        ? "   ·   E-mail: " + data.identificacao.email
        : ""),
  );
  textBlock(
    "Contato de emergência: " + data.identificacao.emergencia.nome + " (" +
      data.identificacao.emergencia.relacao + ") · " +
      data.identificacao.emergencia.telefone,
  );
  textBlock(
    "Local: " + data.identificacao.local_assinatura +
      "   ·   Assinado no dispositivo em: " +
  ...
  textBlock(
    "Categoria do procedimento: " + data.procedimento.modalidades.join("; "),
  );
  textBlock("Região ou regiões informadas: " + data.procedimento.regioes);
  textBlock("Objetivo informado: " + data.procedimento.objetivo);
  textBlock(
    "Plano inicialmente conversado: " +
      data.procedimento.detalhamento_plano_previsto,
  );
  textBlock("Anamnese geral: " + data.procedimento.status_anamnese + ".");
  textBlock(
    "A definição final do dispositivo, indicação aprovada, tipo, quantidade, pontos de entrada, vetores, plano anatômico, técnica e região depende de avaliação profissional e da instrução de uso.",
    { font: bold },
  );
  ...
  for (const item of data.confirmacoes_saude) {
    ...
    const answerLabel = item.resposta === "sim"
      ? "SIM"
      : item.resposta === "nao_sei"
      ? "NÃO SEI"
      : "NÃO";
    textBlock(
      ...
      {
        font: requiresReview ? bold : regular,
        color: requiresReview ? colorRed : colorText,
        after: 5,
      },
    );
  }
  if (data.observacoes_saude) {
    textBlock("Outras informações: " + data.observacoes_saude, { font: bold });
  }

  heading("AUTORIZAÇÕES DE IMAGEM");
  textBlock(
    "Fotografia para prontuário interno: " +
      (data.imagem.foto_prontuario === "sim" ? "AUTORIZADA" : "NÃO AUTORIZADA"),
  );
  textBlock(
    "Divulgação em redes sociais/materiais: " +
      (data.imagem.divulgacao === "sim" ? "AUTORIZADA" : "NÃO AUTORIZADA"),
  );
  textBlock(
    "Antes e depois: " +
      (data.imagem.antes_depois === "sim" ? "AUTORIZADO" : "NÃO AUTORIZADO"),
  );
  textBlock(
    "Forma de exibição: " + data.imagem.forma_imagem.replace(/_/g, " ") + ".",
  );
  textBlock(
    "Uso do primeiro nome: " +
      (data.imagem.primeiro_nome === "sim" ? "AUTORIZADO" : "NÃO AUTORIZADO"),
  );
  textBlock(
    "Depoimento: " +
      (data.imagem.depoimento === "sim" ? "AUTORIZADO" : "NÃO AUTORIZADO"),
  );
  textBlock(
    "As autorizações opcionais podem ser revogadas para o futuro pelo contato da clínica.",
    { size: 8.2, color: colorMuted },
  );
  ...
  textBlock(
    "[CONFIRMADO] Leu integralmente o documento liberado e compreendeu finalidade, limitações, alternativas e riscos descritos.",
  );
  textBlock(
    "[CONFIRMADO] Compreendeu os riscos de irregularidade, assimetria, fio visível ou exposto, infecção, lesão vascular ou nervosa e possível ajuste ou retirada.",
  );
  textBlock(
    "[CONFIRMADO] Declarou que as informações são verdadeiras e que comunicará alterações antes da implantação.",
  );
  textBlock(
    "[CONFIRMADO] Declarou decisão livre e ciência do direito de recusar ou desistir antes do procedimento.",
  );
  textBlock(
    "[CONFIRMADO] Compreendeu que o documento aguarda revisão e assinatura profissional.",
  );
  textBlock(
    "[CONFIRMADO] Está ciente do tratamento de dados para as finalidades clínicas e legais descritas.",
  );

  ensure(145);
  textBlock("Nome digitado como confirmação: " + data.assinatura_digitada, {
    font: bold,
    after: 5,
  });
  if (signature) {
    ...
    let typedSize = 18;
    while (
      typedSize > 11 && italic.widthOfTextAtSize(typedName, typedSize) > 225
    ) typedSize--;
  ...
  textBlock("STATUS: AGUARDANDO REVISÃO PROFISSIONAL", {
    font: bold,
    color: colorRed,
    after: 14,
  });
  textBlock(
    "Data e hora da revisão: ___________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Data do procedimento: ____________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Dispositivo/marca: _______________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Fabricante/modelo: _______________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Material/composição: _____________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Registro Anvisa: __________  Lote: ______________  Validade: _____________________",
    { after: 13 },
  );
  textBlock(
    "Tipo, calibre e comprimento do fio: ______________________________________________",
    { after: 13 },
  );
  textBlock(
    "Quantidade de fios: ______________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Região, pontos de entrada e vetores: ______________________________________________",
    { after: 13 },
  );
  textBlock(
    "Plano anatômico, técnica e introdutor: ____________________________________________",
    { after: 13 },
  );
  textBlock(
    "Anestésico, antisséptico e materiais associados: _________________________________",
    { after: 13 },
  );
  textBlock(
    "Indicação aprovada e instrução de uso conferidas em: _____________________________",
    { after: 13 },
  );
  textBlock(
    "Benefícios, duração, riscos, possível retirada e alternativas explicados:",
    { after: 8 },
  );
  textBlock(
  ...
  textBlock(
    "Cuidados, sinais de alerta, retornos e dúvidas esclarecidos: ______________________",
    { after: 13 },
  );
  textBlock(
    "________________________________________________________________________________",
  ...
  textBlock(
    "Intercorrências/observações: _____________________________________________________",
    { after: 13 },
  );
  textBlock(
    "________________________________________________________________________________",
  ...
  textBlock(
    "Confirmação final da paciente após explicação do plano: ____________________________",
    { after: 13 },
  );
  textBlock(
    "Data/hora: __________________  Nova manifestação gerada:  [  ] Sim  [  ] Não",
    { after: 13 },
  );
  textBlock(
    "Orientações e registro do dispositivo entregues ao paciente:  [  ] Sim  [  ] Não",
    { after: 18 },
  );
  textBlock("Ana Maria Costa Jacob · Farmacêutica · CRF/MG 40880", {
    font: bold,
    after: 22,
  });
  textBlock(
    "Assinatura profissional: _________________________________________________________",
    { after: 5 },
  );
  ...
  for (let index = 0; index < pages.length; index++) {
    ...
    current.drawLine({
      ...
      thickness: 0.5,
      color: rgb(232 / 255, 220 / 255, 205 / 255),
    });
    current.drawText(
      pdfText(
        "Ana Maria Jacob Estética · " + TERM_VERSION + " · " +
          code.slice(0, 16),
      ),
      {
        x: margin,
...
Deno.serve(async (req: Request) => {
  ...
  if (!RELEASED) {
    return fail(
      ...
      "Este documento está temporariamente indisponível para envio.",
      423,
    );
  }
  if (
  ...
  if (text(payload.website, 100)) {
    return fail(req, "invalid_submission", "Não foi possível validar o envio.");
  }
  if (!validUuid(payload.idempotency_key)) {
    return fail(
      req,
      "invalid_idempotency",
  ...
  try {
    ...
    if (
      previous &&
      (previous.tipo !== TYPE || previous.versao_termo !== TERM_VERSION)
    ) {
      return fail(
        req,
        "invalid_idempotency",
    ...
    if (previous?.status === "recebido") {
      ...
      return json(req, {
        ...
        pdf_nome: "Pre-Avaliacao-Fios-PDO-" +
          text(previous.codigo_verificacao, 8) + ".pdf",
        idempotente: true,
      });
    }
    if (previous?.status === "processando") {
      ...
      if (Number.isFinite(updatedAt) && Date.now() - updatedAt < 2 * 60_000) {
        return fail(
          req,
          "already_processing",
          "Este envio ainda está sendo processado. Aguarde alguns segundos e tente novamente.",
          409,
        );
      }
      recovery = previous;
  ...
  if (
    payload.tipo !== TYPE || payload.versao_termo !== TERM_VERSION ||
    payload.termo_sha256 !== TERM_SHA256
  ) {
    return fail(
      req,
      "term_version_mismatch",
      "A versão do termo mudou. Atualize a página e leia novamente.",
      409,
    );
  }
  ...
  if (!termText || await sha256(termText) !== TERM_SHA256) {
    return fail(
      ...
      "O texto integral do termo não pôde ser validado. Atualize a página.",
      409,
    );
  }
  ...
  const now = Date.now();
  if (
    !Number.isFinite(startedAt) || !Number.isFinite(signedAt) ||
    signedAt < startedAt || now - startedAt < 5_000 ||
    now - startedAt > 86_400_000 || signedAt > now + 300_000 ||
    now - signedAt > 86_400_000
  ) {
    return fail(
      req,
      "invalid_timestamps",
      "A data do envio não pôde ser validada. Atualize a página.",
    );
  }
  ...
  if (
    nome.length < 5 || !validCpf(cpf) || !/^\d{10,11}$/.test(telefone) ||
    (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
  ) {
    return fail(
      req,
      "invalid_identity",
      "Revise os dados de identificação e tente novamente.",
    );
  }
  ...
  if (!data) {
    return fail(
      req,
      "invalid_answers",
      "Revise as respostas obrigatórias e tente novamente.",
    );
  }
  ...
  if (!signature || !isPng(signature)) {
    return fail(
      req,
      "invalid_signature",
      "A assinatura não pôde ser validada.",
    );
  }
  ...
  if (
    ...
    payload.assinatura_sha256 !== signatureHash
  ) {
    return fail(
      req,
      "hash_mismatch",
      "A integridade da assinatura não pôde ser confirmada.",
    );
  }

  const expectedClientCode = await sha256(
    idempotencyKey + "|" + TYPE + "|" + TERM_VERSION + "|" + TERM_SHA256,
  );
  if (
    !validHash(payload.codigo_verificacao) ||
    payload.codigo_verificacao !== expectedClientCode
  ) {
    return fail(
      req,
      "invalid_code",
      "O código do documento não pôde ser validado.",
    );
  }
  ...
  try {
    const rate = await admin(
      "/rest/v1/documentos_clinicos?select=id&origem_hash=eq." + originHash +
        "&status=eq.recebido&recebido_em=gte." + encodeURIComponent(since) +
        "&limit=6",
    );
    if (rate.ok) {
      ...
      if (Array.isArray(rows) && rows.length >= 5) {
        return fail(
          req,
          "rate_limited",
          "Muitos envios foram feitos desta conexão. Aguarde 15 minutos e tente novamente.",
          429,
        );
      }
    }
  ...
  const signaturePath = "tcle-fios-pdo/" + idempotencyKey +
    "/assinatura.png";
  const pdfPath = "tcle-fios-pdo/" + idempotencyKey + "/documento.pdf";
  ...
  try {
    ...
    return fail(
      ...
      "Não foi possível preparar a cópia digital agora. Tente novamente.",
      503,
    );
  }
  if (!pdf.byteLength || pdf.byteLength > MAX_PDF_BYTES || !isPdf(pdf)) {
    return fail(
      req,
      "pdf_generation_failed",
      "A cópia digital não pôde ser validada. Tente novamente.",
      503,
    );
  }
  ...
  try {
    if (recovery) {
      const oldSignaturePath = text(recovery.assinatura_path, 500) ||
        signaturePath;
      ...
      if (!recoveryStatus || !recoveryUpdatedAt) {
        throw new Error("recovery_state_missing");
      }
      ...
      if (!recovered.ok) {
        throw new Error("database_recovery_failed_" + recovered.status);
      }
      ...
      if (!Array.isArray(records) || !records.length) {
        return fail(
          req,
          "already_processing",
          "Este envio foi retomado em outra solicitação. Aguarde alguns segundos e tente novamente.",
          409,
        );
      }
        ...
        if (inserted.status === 409) {
          ...
          if (previous?.status === "recebido") {
            ...
            return json(req, {
              ...
              codigo: previous.codigo_verificacao,
              recebido_em: previous.recebido_em,
              pdf_url: previousPdfPath
                ? await signedPdfUrl(previousPdfPath)
                : null,
              pdf_nome: "Pre-Avaliacao-Fios-PDO-" +
                text(previous.codigo_verificacao, 8) + ".pdf",
              idempotente: true,
    ...
    if (!updated.ok) {
      throw new Error("database_finalize_failed_" + updated.status);
    }
    ...
    return fail(
      req,
      "temporary_error",
      "Não foi possível concluir o envio agora. Seus dados continuam na tela; tente novamente.",
      503,
    );
  }
});
