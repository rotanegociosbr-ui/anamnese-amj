...
async function existing(idempotencyKey: string): Promise<JsonRecord | null> {
  const response = await admin(
    "/rest/v1/documentos_clinicos?select=id,status,codigo_verificacao,recebido_em,updated_at,pdf_path,assinatura_path&idempotency_key=eq." +
...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (recovery) {
      ...
      const recovered = await admin("/rest/v1/documentos_clinicos?id=eq." + encodeURIComponent(recordId), {
        method: "PATCH",
