...
const TYPE = "tcle_peeling_quimico";
const TERM_VERSION = "2026-08-18-v1";
const TERM_SHA256 =
  "3f04db1af1f0e984385862abdd0b1b93ae34ebfd49f09587f541c317022b530b";
const RELEASED = true;
...
const HEALTH_QUESTIONS = Object.freeze([
  "Você está gestante, amamentando ou existe possibilidade de gestação",
  "Você tem alergia ou já apresentou reação a ácidos, cosméticos ou peelings",
  "Há ferida, irritação, queimadura solar, dermatite, infecção ou herpes na região pretendida",
  "Você já teve herpes nos lábios ou no rosto, mesmo sem lesão ativa agora",
  "Você usa ou usou recentemente isotretinoína, retinoides, ácidos ou medicamento que aumente sensibilidade ao sol",
  "Você usa atualmente algum medicamento, suplemento ou produto dermatológico",
  "Você tem diabetes, imunossupressão, doença autoimune ou outra condição que possa alterar a cicatrização",
  "Você já apresentou queloide, cicatrização ruim ou manchas após inflamação ou procedimento",
  "Você teve exposição solar intensa, bronzeamento ou queimadura solar recentemente",
  "Você realizou recentemente peeling, laser, microagulhamento, cirurgia, depilação, cera ou outro procedimento na área",
  "Há pinta, ferida persistente, lesão suspeita, câncer de pele prévio ou radioterapia na região",
]);

const ALLOWED_MODALITIES = new Set([
  "Peeling químico superficial não fenólico",
]);
...
async function buildCanonicalPdf(
  ...
  pdf.setTitle(
    "TCLE - Peeling Químico Superficial - " + data.identificacao.nome,
  );
  pdf.setSubject(
  ...
  textBlock("PEELING QUÍMICO SUPERFICIAL NÃO FENÓLICO", {
    font: bold,
    size: 11,
    lineHeight: 15,
    after: 4,
  ...
  textBlock(
    "A definição final do produto, ativos, concentração, pH, profundidade, tempo de contato, remoção ou neutralização e região depende de avaliação profissional e da documentação aplicável ao produto proposto.",
    { font: bold },
  );
  ...
  textBlock(
    "[CONFIRMADO] Compreendeu que peeling produz lesão química controlada e pode causar queimadura, infecção, alteração de pigmentação, cicatriz e lesão ocular.",
  );
  textBlock(
    "[CONFIRMADO] Declarou que as informações são verdadeiras e que comunicará alterações antes da aplicação.",
  );
  textBlock(
  ...
  textBlock(
    "Produto/marca: __________________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Fabricante/fornecedor: ____________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Ativos/composição: _______________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Lote: __________________  Validade: __________  Regularização sanitária: __________",
    { after: 13 },
  );
  textBlock(
    "Concentração e pH, quando aplicável: ______________________________________________",
    { after: 13 },
  );
  textBlock(
    "Indicação e região: ______________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Fototipo, integridade da pele e fatores de risco: _________________________________",
    { after: 13 },
  );
  textBlock(
    "Profundidade pretendida: _________________________________________________________",
    { after: 13 },
  );
  textBlock(
    "Camadas/passadas e tempo de contato: ______________________________________________",
    { after: 13 },
  );
  textBlock(
    "Método de remoção ou neutralização: _______________________________________________",
    { after: 13 },
  );
  textBlock(
    "Cuidados antes/depois e suspensão/reintrodução de produtos: ______________________",
    { after: 8 },
  );
  textBlock(
  ...
  textBlock(
    "Benefícios, limitações, riscos, alternativas e dúvidas explicados: _______________",
    { after: 13 },
  );
  textBlock(
    "________________________________________________________________________________",
  ...
  textBlock(
    "Orientações e registro do protocolo entregues ao paciente:  [  ] Sim  [  ] Não",
    { after: 18 },
  );
...
Deno.serve(async (req: Request) => {
  ...
  if (!RELEASED) {
    return fail(
      ...
      "Este documento não está disponível para recebimento.",
      423,
    );
  }
  if (
  ...
  try {
    ...
    if (previous?.status === "recebido") {
      ...
      return json(req, {
        ...
        pdf_nome: "Pre-Avaliacao-Peeling-Quimico-Superficial-" +
  ...
  const signaturePath = "tcle-peeling-quimico/" + idempotencyKey +
    "/assinatura.png";
  const pdfPath = "tcle-peeling-quimico/" + idempotencyKey + "/documento.pdf";
