...
begin;
create temporary table progressive_draft_context(
...
grant select,update on progressive_draft_context to service_role;
do $fixture$
...
begin
 ...
 values(p,c,'TESTE TRANSACIONAL RASCUNHO '||p::text,u,p::text);
 ...
 insert into progressive_draft_context values(c,u,p,product,null,null,gen_random_uuid(),gen_random_uuid());
end;$fixture$;
...
begin
 select * into f from progressive_draft_context;
 ...
 update progressive_draft_context set protocol_id=(a->>'id')::uuid,version=(a->>'version')::integer;
 select * into f from progressive_draft_context;
 ...
 a:=public.prontuario_salvar_rascunho_com_estoque(f.clinic_id,f.actor_id,'owner','supabase_auth',
  ...
  '{"clinical_photography":true}',gen_random_uuid());
 update progressive_draft_context set version=(a->>'version')::integer;
end;$draft$;
reset role;
...
select 'clinic-media',clinic_id::text||'/'||protocol_id::text||'/'||photo_id::text||'.jpg',
 '{"mimetype":"image/jpeg","size":1}'::jsonb from progressive_draft_context
union all
