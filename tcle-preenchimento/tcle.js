...
function collectData(signedAt) {
  ...
  return {
    ...
    procedimento: {
      ...
      detalhamento_volume_previsto: form.elements.detalhamento_volume_previsto.value.trim(),
    ...
    declaracoes: {
      ...
      risco_vascular: form.elements.aceite_risco_vascular.checked,
      ciencia_hialuronidase_urgencia: form.elements.aceite_hialuronidase.checked,
...
function showSuccess(response, data) {
  ...
  state.pdfFilename = String(response.pdf_nome || '').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 100) ||
    'TCLE-Preenchimento-Facial-' + safeFilename(data.identificacao.nome) + '-' + response.codigo.slice(0, 8) + '.pdf';
  ...
  const whatsappText = 'Olá, Ana! Concluí o TCLE de preenchimento facial pelo site. Nome: ' + data.identificacao.nome + '. Código: ' + response.codigo.slice(0, 16) + '.';
