...
function renderHealthQuestions() {
  ...
  target.innerHTML = HEALTH_QUESTIONS.map((question, index) => {
    ...
    return '<fieldset class="health-question" data-question="' + number + '">' +
      ...
      '<textarea name="saude_' + number + '_detalhe" rows="3" maxlength="600" disabled></textarea></label>' +
      '</fieldset>';
  }).join('');
  ...
  target.addEventListener('change', (event) => {
    ...
    card.classList.toggle('attention', isYes);
    detail.disabled = !isYes;
    detail.required = isYes;
    if (!isYes) {
      detail.value = '';
      detail.removeAttribute('aria-invalid');
    } else {
      setTimeout(() => detail.focus(), 0);
    }
  });
}
...
function setupImagePermissions() {
  ...
  function update() {
    ...
    radios.forEach((radio) => { radio.disabled = !allowed; });

    if (!allowed) {
      form.elements.antes_depois.value = 'nao';
      form.elements.primeiro_nome.value = 'nao';
      form.elements.depoimento.value = 'nao';
      form.elements.forma_imagem.value = 'nao_aplicavel';
      [...form.querySelectorAll('input[name="forma_imagem"]')].forEach((radio) => { radio.required = false; });
    } else {
...
function validateForm() {
  ...
  if (!form.checkValidity()) {
    ...
    return { ok: false, message: 'Revise os campos obrigatórios destacados antes de enviar.', element: invalid };
  }
  if (!validCpf(form.elements.cpf.value)) return { ok: false, message: 'Informe um CPF válido.', element: form.elements.cpf };
  if (!isAdult(form.elements.nascimento.value)) return { ok: false, message: 'O preenchimento online está disponível somente para maiores de 18 anos. Fale com a clínica para uma avaliação específica.', element: form.elements.nascimento };
  ...
  const regions = [...form.querySelectorAll('input[name="regioes"]:checked')];
  if (!regions.length && !form.elements.regiao_outra.value.trim()) {
    document.getElementById('erro-regioes').textContent = 'Escolha ao menos uma região ou preencha “Outra região”.';
    return { ok: false, message: 'Informe ao menos uma região de interesse.', element: form.querySelector('input[name="regioes"]') };
  }
  document.getElementById('erro-regioes').textContent = '';
  ...
  if (!state.signed) return { ok: false, message: 'Assine antes de enviar: desenhe na área ou use o botão com o nome digitado.' };
...
function collectData(signedAt) {
  const regions = [...form.querySelectorAll('input[name="regioes"]:checked')].map((input) => input.value);
  if (form.elements.regiao_outra.value.trim()) regions.push('Outra: ' + form.elements.regiao_outra.value.trim());
...
function showSuccess(response, data) {
  ...
  state.pdfFilename = String(response.pdf_nome || '').replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 100) ||
    'TCLE-Toxina-' + safeFilename(data.identificacao.nome) + '-' + response.codigo.slice(0, 8) + '.pdf';
  ...
  const whatsappText = 'Olá, Ana! Concluí o TCLE de toxina botulínica pelo site. Nome: ' + data.identificacao.nome + '. Código: ' + response.codigo.slice(0, 16) + '.';
