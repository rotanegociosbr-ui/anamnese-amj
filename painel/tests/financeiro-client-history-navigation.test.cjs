...
test('canonical history stays owner-only, follows Clientes, and is not duplicated in secondary navigation', () => {
  ...
  const context = {};
  vm.runInNewContext(shellSource.slice(start, end) + '\nthis.navigation={ROUTES,PRIMARY_ORDER,SECONDARY_ORDER};', context);
  const { ROUTES, PRIMARY_ORDER, SECONDARY_ORDER } = context.navigation;
  ...
  assert.equal(SECONDARY_ORDER.includes('prontuarios'), false);
  assert.equal(PRIMARY_ORDER.filter(route => route === 'prontuarios').length, 1);
  assert.match(shellSource, /data-shell-route="prontuarios">Prontuários e fotos<\/button>/);
  assert.match(shellSource, /navButton\('inicio', true\) \+ navButton\('crm', true\) \+ navButton\('procedimentos', true\) \+ navButton\('agenda', true\)/);
});
